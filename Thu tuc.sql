...
IF(@MA_NUOC_SX IN(SELECT MA_NUOC_SX FROM Nuoc_SX) AND @MA_LOAI_HANG IN(SELECT MA_LOAI_HANG FROM Loai_Hang) AND @MA_KHO IN(SELECT MA_KHO FROM Danh_Muc_Kho))
     INSERT INTO Danh_Muc_Hang_Hoa(MA_HANG,TEN_HANG,MA_NUOC_SX,KICH_THUOC,MA_LOAI_HANG,DON_VI_TINH,LUONG_TON,MA_KHO) VALUES(@MA_HANG,@TEN_HANG,@MA_NUOC_SX,@KICH_THUOC,@MA_LOAI_HANG,@DON_VI_TINH,@LUONG_TON,@MA_KHO)
END
...
BEGIN
IF(@MA_NUOC_SX IN(SELECT MA_NUOC_SX FROM Nuoc_SX) AND @MA_LOAI_HANG IN(SELECT MA_LOAI_HANG FROM Loai_Hang) AND @MA_KHO IN(SELECT MA_KHO FROM Danh_Muc_Kho))
     UPDATE Danh_Muc_Hang_Hoa SET MA_HANG=@MA_HANG,
	 TEN_HANG=@TEN_HANG,
	 MA_NUOC_SX=@MA_NUOC_SX,
	 KICH_THUOC=@KICH_THUOC,
	 MA_LOAI_HANG=@MA_LOAI_HANG,DON_VI_TINH,LUONG_TON,MA_KHO) VALUES(,,,,,@DON_VI_TINH,@LUONG_TON,@MA_KHO)
END